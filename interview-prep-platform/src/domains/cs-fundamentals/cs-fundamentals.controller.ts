...
import { csFundamentalsService } from './cs-fundamentals.service';
...
export class CsFundamentalsController {
    async handleList(request: NextRequest): Promise<NextResponse> {
        try {
            ...
            const result = await csFundamentalsService.getAll({
                page,
                limit,
    ...
    async handleGetById(_req: NextRequest, { params }: { params: { id: string } }): Promise<NextResponse> {
        try {
            ...
            const data = await csFundamentalsService.getById(id);
    ...
    async handleGetCategories(_req: NextRequest): Promise<NextResponse> {
        try {
            const data = await csFundamentalsService.getCategories();
            return ApiResponse.success(data);
        } catch {
...
export const csFundamentalsController = new CsFundamentalsController();
